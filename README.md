...
> rolling restart of the cluster to occur. If you do not want the cluster to restart you can specify `restart_node=false`
> either globally (as --extra-vars, or on a host-by-host basis in the `hosts.ini` file). If you make changes to the node
> config and do not perform a restart then you may leave your `rpk` config (and this may inhibit subsequent executions of
> the playbook). If you re-run the playbook it will overwrite any configurations with those specified in the playbook, so
